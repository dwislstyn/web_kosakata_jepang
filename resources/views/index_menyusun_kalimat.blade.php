...
<head>
    ...
    <link rel="stylesheet" href="{{ asset('style/style_menyusun_kalimat.css') }}">
    
    <title>Menyusun Kalimat</title>
</head>
<body>
    ...
    <div class="d-flex flex-column flex-md-row" style="padding-bottom: 10px;">
        <div class="container index-kosakata">
            ...
            <div class="container" style="margin-bottom: 3rem;">
                <p class="label-card-kosakata">Test Menyusun Kalimat</p>
            </div>
                ...
                <div class="row area-scroll-kata">
                    ...
                    @foreach ($data as $listSoal)
                        <div class="form-group">
                            <div class="row">
                                <label for="nomor{{ $nomor }}">{{ $nomor.'. '.$listSoal->soal }}</label>
                            </div>

                            @php
                                $arrCheck = explode('/',$listSoal->soal);
                            @endphp
                            
                            @foreach ($arrCheck as $keyCheck => $listCheck)
                                <div class="form-check form-check-inline">
                                    <input class="form-check-input check-set check-{{ $nomor }}" type="checkbox" value="{{ $listCheck }}" data-nomor={{ $nomor }} id="checkBox{{ $nomor.$keyCheck }}">
                                    <label class="form-check-label" for="checkBox{{ $nomor.$keyCheck }}">{{ $listCheck }}</label>
                                </div>
                            @endforeach

                            <input type="text" class="form-control txtNomor" name="txtNomor{{ $nomor }}" id="txtNomor{{ $nomor }}">
...
<script>
    ...
    $('.check-set').click(function() {
        ...
        selectedValues[nomor] = selectedValues[nomor] ?? [];
    
        if (isChecked) {
            selectedValues[nomor].push(value)
        } else {
            selectedValues[nomor] = selectedValues[nomor].filter(function(item){
                return item !== value;
            });
        }

        $('#formSoal').find('#txtNomor'+nomor).val(selectedValues[nomor].join(''));
    });
    ...
    $('#btnSubmit').click(function(){
        ...
        $.ajax({
            type: 'POST',
            url: "{{ route('submitMenyusunKalimat') }}",
            async: false,
            dataType: 'json',
